...
function casanova_payment_intent_create(array $data) {
  ...
  $row = [
    ...
    'provider'     => strtolower(trim((string)($data['provider'] ?? 'redsys'))),
    'method'       => strtolower(trim((string)($data['method'] ?? 'card'))),
    'order_redsys' => !empty($data['order_redsys']) ? (string)$data['order_redsys'] : null,
    'provider_payment_id' => !empty($data['provider_payment_id']) ? (string)$data['provider_payment_id'] : null,
    'provider_reference'  => !empty($data['provider_reference']) ? (string)$data['provider_reference'] : null,
...
function casanova_payment_intent_get_by_provider_payment_id(string $provider_payment_id) {
  global $wpdb;
  $table = casanova_payments_table();
  $provider_payment_id = trim($provider_payment_id);
  if ($provider_payment_id === '') return null;

  return $wpdb->get_row(
    $wpdb->prepare("SELECT * FROM {$table} WHERE provider_payment_id=%s LIMIT 1", $provider_payment_id)
  );
}

function casanova_payment_intent_get_by_provider_reference(string $provider_reference) {
  global $wpdb;
  $table = casanova_payments_table();
  $provider_reference = trim($provider_reference);
  if ($provider_reference === '') return null;

  return $wpdb->get_row(
    $wpdb->prepare("SELECT * FROM {$table} WHERE provider_reference=%s LIMIT 1", $provider_reference)
  );
}
...
function casanova_payment_intent_update(int $id, array $fields): bool {
  ...
  $allowed = [
    ...
    'provider','method','order_redsys','provider_payment_id','provider_reference',
    'payload','mail_cobro_sent_at','mail_expediente_sent_at',
